...
def compute_rollout_correction_and_rejection_mask(
    ...
    Args:
        ...
        rollout_is_batch_normalize: Whether to normalize IS weights to have mean=1.0 per batch.
            Default: False.
        rollout_token_veto_threshold: Per-token catastrophic-ratio veto. If set to a positive
            float, any sequence with at least one token whose ``π_train/π_rollout`` falls below
            this value has its entire ``modified_response_mask`` row zeroed. Implements the
            token-level veto from DR.Kernel's MRS (paper value: 1e-4). Default: None (disabled).
    ...
    # Step 3.5: Per-token catastrophic-ratio veto. Sequences containing any token
    # with π_train/π_rollout below `rollout_token_veto_threshold` have their full
    # response_mask zeroed for the gradient step. Composes with `rollout_rs` via
    # mask multiplication. Ported from DR.Kernel's MRS (verl_patch/.../mismatch_helper.py
    # Step 4). The "veto" semantics are deliberate: per-token rejection (which is
    # what `rollout_rs=token_k1` does) leaves the surrounding tokens of a
    # catastrophic sequence in the loss; MRS argues that whole trajectory is
    # untrustworthy.
    if rollout_token_veto_threshold is not None:
        if rollout_token_veto_threshold <= 0:
            raise ValueError(
                f"rollout_token_veto_threshold must be positive, got {rollout_token_veto_threshold}."
            )
        ...
        metrics["rollout_veto_seq_fraction"] = has_catastrophic.float().mean().item()
        metrics["rollout_veto_catastrophic_token_fraction"] = verl_F.masked_mean(
            catastrophic_tokens.float(), response_mask
        ).item()
    else:
        metrics["rollout_veto_seq_fraction"] = 0.0
        metrics["rollout_veto_catastrophic_token_fraction"] = 0.0
