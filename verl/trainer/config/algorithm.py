...
class RolloutCorrectionConfig(BaseConfig):
    ...
    Args:
        ...
        rollout_token_veto_threshold (Optional[float]): Per-token catastrophic-ratio veto.
            When set to a positive float, any sequence containing a token whose
            ``π_train / π_rollout`` drops below this value has its entire ``response_mask``
            zeroed for the gradient step. Operates independently of ``rollout_rs`` (composes
            with it via mask multiplication). Implements the token-level veto from DR.Kernel's
            MRS (paper value: 1e-4). Default: None (disabled).
