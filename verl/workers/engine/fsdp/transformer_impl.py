...
class FSDPEngine(BaseEngine):
    ...
    def save_checkpoint(
        ...
        **kwargs,
        ...
        moved_to_gpu_for_save = self._is_offload_param or origin_module_device == "cpu"
        if moved_to_gpu_for_save:
            load_fsdp_model_to_gpu(self.module)
        ...
        torch.distributed.barrier()
        # Restore whatever device the params were on before save. Without this,
        # FSDP2 + offload_policy ends with params on NPU but FSDP2's
        # CPUOffloadPolicy still emitting CPU-sharded grads, which trips a
        # device-mismatch assert in foreach_reduce on the next backward.
        if moved_to_gpu_for_save:
            offload_fsdp_model_to_cpu(self.module)
